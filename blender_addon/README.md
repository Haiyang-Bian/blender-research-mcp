...
`artifacts/blender-research-mcp-addon-0.3.0.zip`. Install that ZIP in Blender
...
The current surface includes authenticated ping, context snapshots, exact object
inspection, focus-independent GPU off-screen viewport capture, and one
transaction-scoped absolute local-scale operation. Blender may be obscured by
another window, but it must remain running with a `VIEW_3D`; minimized capture
is not guaranteed. The add-on returns a structured GPU error instead of a black
image when capture is unavailable.
